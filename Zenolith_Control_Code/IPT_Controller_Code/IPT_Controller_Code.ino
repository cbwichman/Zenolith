...
unsigned long lastXUpdate, lastYUpdate, lastZUpdate, lastUpdate;
...
void loop() {
  if ((millis() - lastUpdate) > BNO055_SAMPLERATE_DELAY_MS) {
    updateX();
    //Serial.println(" update x called ");
    setxPoint = 0;
    updateY();
    setyPoint = 0;
    updateZ();
    ...
    // might need to remove this if statement... possibly acting as a while loop?
    if (millis() > 10000) {
      //int s = int(outputVal - 255.0);
      Serial.println(" X ouput value ");
      Serial.println(outputValX);
      int xSpeed;
      if (outputValX < 0)xSpeed = outputValX; //-180 +
        //if (xSpeed < -255)xSpeed = -255;
      else xSpeed = outputValX;
        //if (xSpeed > 255)xSpeed = 255;
  
  
      int ySpeed;
      if (outputValY < 0)ySpeed = -200 - outputValY;
        if (ySpeed < -255)ySpeed = -255;
      else ySpeed = 200 + outputValY;
        if (ySpeed > 255)ySpeed = 255;
  
      int zSpeed;
      if (outputValZ < 0)zSpeed = -199 - outputValZ;
        if (zSpeed < -255)zSpeed = -255;
      else zSpeed = 199 + outputValZ;
        if (zSpeed > 255)zSpeed = 255;
      ...
      FM(0, FLYMOTOR_X_DIR, FLYMOTOR_X_SPEED);
      FM(0, FLYMOTOR_Y_DIR, FLYMOTOR_Y_SPEED);
      FM(0, FLYMOTOR_Z_DIR, FLYMOTOR_Z_SPEED);
      lastUpdate = millis();
    }
  }
...
void FM(int mSpeed, int motorDirPin, int motorSpeedPin) {
  ...
  if (mSpeed < 0) {
    analogWrite( motorSpeedPin,  abs(mSpeed));
    digitalWrite( motorDirPin, 0 ); // should be digital
  }
  else if (mSpeed > 0) {
    analogWrite( motorSpeedPin,  255 - abs(mSpeed));
    digitalWrite( motorDirPin,  255);
  } else {
